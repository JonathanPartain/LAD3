...
public class Gothenburg {
    ...
    /** A tram network object for a small part of the Gothenburg tram network. This network can be used with TramFider. **/
...
//        findGBG(23*60+50,"Kor","Chal");
    ...
    private static void findGBG(int time, String from, String to){
        ...
        deps = FastFinder.findRoute(GOTHENBURG, time, sfrom, sto);
        TramFinder.printTravel(deps);

        int time2 = deps==null ? -1 : (deps.length == 0) ? 0 : deps[deps.length-1].getArrivalTime()-time;

        System.out.println();
        System.out.println("Times: " + (time1) + " vs " + (time2));
        System.out.println(time1==time2 ? "Implementations agree :)" : "ERROR: Implementations disagree :(");
